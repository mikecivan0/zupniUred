...
include_once '../../sql/financije/dnevnici/nadjiGodineZaPlavi.php';
...
$title = 'Plavi blagajnički dnevnik';
...
<div class="row mt40">	
	 	...
	 	if(!empty($godine)): ?>
				...
				<ul>			
					<?php
						...
						$mjeseci = $godineIMjeseci->nadjiMjeseceZaPlavi($godina);	
						?>
	...
	<div class="large-10 columns">
		...
		<fieldset class="panel">
			<legend style="font-weight: normal; color: blue;">
				Župa <?= $zupa->nazivZupe ?> - <b><?= strtolower(Alati::hrvMjesec($mjesecTransakcije)) ?> <?= $godinaTransakcije ?>.</b> godine		
			</legend>
			<?php	
				echo (empty($transakcije)) ? Financije::pocetakTablice('Primitak B-1','Izdatak D-1') . Financije::krajTablice() : null; //ako nema podataka onda kreiraj početak tablice		
				foreach ($transakcije as $transakcija) {
					...
					echo ($count==1) ? Financije::pocetakTablice('Primitak B-1','Izdatak D-1') : null; //ako ima podataka onda kreiraj početak tablice ako je count 1
					...
					if($count==24){//ako se dođe do kraja stranice
						...
						Financije::zbrajanjeNaKrajuLista(Alati::hrIznos(number_format($ukupnoB , 2)), Alati::hrIznos(number_format($ukupnoD , 2)),
														 Alati::hrIznos(number_format($prosloStanjeB , 2)), Alati::hrIznos(number_format($prosloStanjeD , 2)), 
														 Alati::hrIznos(number_format($ukupnoB + $prosloStanjeB , 2)), Alati::hrIznos(number_format($ukupnoD + $prosloStanjeD , 2)));
		...
		<div class="large-2 columns">
			<a target="_blank" id="printIcon" href="<?= $putanjaApp  . 'financije/ispis/plavi.php?id=' . $_GET['id'] . 
		...
		<div class="large-2 columns pt40">
			...
			<?= Html::Input(null, 'hidden', 'hfPath', 'hfPath',null,null,$putanjaApp  . 'financije/ispis/plavi.php?id=' . $_GET["id"] . 
