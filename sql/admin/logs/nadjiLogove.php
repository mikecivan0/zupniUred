...
$_POST["hfUserId"] = (isset($_POST["hfUserId"])) ? $_POST["hfUserId"] : $_GET["user"];

$izraz = $veza -> prepare("select count(id) as ukupno from prijave where user_id=:id");
...
$sql = "select p.*,concat(o.ime,' ',o.prezime) as osoba, u.username from 
						   ...
						   inner join osobe o on u.osoba_id=o.id where p.user_id=:user_id order by p.id desc limit " . $offset . ", 20;";
$izraz = $veza -> prepare($sql);
	$izraz -> bindParam(':user_id', $_POST["hfUserId"]);
	$izraz -> execute();
