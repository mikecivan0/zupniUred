...
if($mjesec==1){//ako se gleda siječanj onda dohvati stanja od prošle godine za A i C
	...
	$izraz = $veza -> prepare("select SUM(t.iznos) from 
						   ...
						   where t.zupa_id=:id and s.grupa_id=1 and YEAR(t.datum)=:godina and MONTH(t.datum) < :mjesec;");
	$izraz -> bindParam(':id', $_GET["id"]);
	$izraz -> bindParam(':godina', $godina);
	$izraz -> bindValue(':mjesec', $mjesec);
	$izraz -> execute();
	$prosloStanjeA = $izraz -> fetch(PDO::FETCH_COLUMN);
	
	
	$izraz = $veza -> prepare("select SUM(t.iznos) from 
						   transakcije t inner join svrhe s on t.svrha_id=s.id
						   where t.zupa_id=:id and s.grupa_id=3 and YEAR(t.datum)=:godina and MONTH(t.datum) < :mjesec;");
	$izraz -> bindParam(':id', $_GET["id"]);
	$izraz -> bindParam(':godina', $godina);
	$izraz -> bindValue(':mjesec', $mjesec);
	$izraz -> execute();
	$prosloStanjeC = $izraz -> fetch(PDO::FETCH_COLUMN);	
}
